...
monthly_subscriptions AS (
    SELECT
        ...
        plan_name,
        pricing,
        {{ date_trunc_month('starts_at') }} AS start_month,
        {{ date_trunc_month('ends_at') }} AS end_month
    FROM
        {{ dim_subscriptions }}
    WHERE
...
subscription_periods AS (
    SELECT
        ...
        CASE
            ...
            WHEN end_month IS NULL THEN DATE(DATEADD('month', 1, {{ date_trunc_month('current_date') }}))
            ELSE end_month
        END AS end_month
    FROM
...
final AS (
    ...
    WHERE
        date_month <= {{ date_trunc_month('current_date') }}
)
